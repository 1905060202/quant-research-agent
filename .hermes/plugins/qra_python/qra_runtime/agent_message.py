"""QRA 内核会话间消息 skill（prime agent-message skill 移植，v0.7.2@83a0f9f9）。

prime 中路由与发送者身份都在 TypeScript 守护进程；QRA 中在宿主插件
（qra_python）的 dispatch 表里：agent_message.send 写入接收方会话目录下的
收件箱文件（QRA 实现），agent_message.list_agents 返回宿主注册表里的
parent/兄弟/子女。内核侧只调 host 桥，与 prime 一字不差。
"""
...
from qra_runtime import host_request

ReceiverRole = Literal["parent", "sibling", "child"]
_MESSAGE_DISPLAY_MIME = "application/vnd.qra.agent-message+json"
